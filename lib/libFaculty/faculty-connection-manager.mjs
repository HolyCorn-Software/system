...
export const faculty_platform_symbol = Symbol(`FacultyConnectionManager.prototype.faculty_platform`)
...
export class FacultyConnectionManager {
    ...
    constructor(faculty_platform) {
        ...
        faculty_platform.base.channel.remote.setNetworkingEnabled(true);
    ...
    addConnection(connection) {
        ...
        if (this[connections_symbol].some(
            ...
            let prev = this[connections_symbol].filter(x => x.descriptor.name === connection.descriptor.name)[0]
        ...
        setTimeout(() => connection.rpc.$rpc.events.on('$remote-event', (type, ...data) => {
            //Call a direct emit() so as to prevent recursion
            this.events.emit0(type, { faculty: connection }, ...data)
        }), 2000);
    ...
    async connect(name) {
        ...
        try {
            ...
            let getCredentials = async () => {
                credentials = await this[faculty_platform_symbol].base.channel.remote.getFacultyRPCInfo(name)

                if (credentials === null) {
                    throw new Exception(`Credentials not found for faculty '${name}'. Perhaps faculty has not yet registered at BasePlatform`, { code: 'error.system.unplanned' })
                }
            }
            ...
            const conn = async () => {
                return await FacultyFacultyRPCClient.connect(credentials, this[faculty_platform_symbol])
            }
...
export class ConnectionManagerEventEmitter extends WildcardEventEmitter {
    ...
    async emit(type, ...data) {
        for (let connection of this[faculty_connection_manager_symbol][connections_symbol]) {
            setImmediate(() => connection.rpc.$rpc.events.emit(type, ...data)); //For some reason, if we don't wait, not all faculties get the event
        }
