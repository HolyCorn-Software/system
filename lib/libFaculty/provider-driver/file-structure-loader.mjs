...
export class FileStructureLoader {
            ...
            for (let struct of structure) { //So, for each of the entities in the structure definition, we check that the path complies
                if (typeof struct === 'string') {
                    if (
                        ...
                        throw new Error(`${struct.yellow} was not found at ${path.yellow}, as intended`)
                    }

                } else {
                    //Second situation, the entity is a folder
                    if (typeof struct.name === 'undefined' || !Array.isArray(struct.files)) {
                        throw new Error(`Bad structure definition found ${JSON.stringify(struct)}. We either use strings of arrays (representing files) or objects with the 'files' and 'name' property (representing directories). For example: ${JSON.stringify(example)} `)
                    }
     ...
     * @returns {{data: {[key:string]: LoadedDirectory}, errors: {[key:string]: Error}}}
     */
    loadAll() {
            ...
            get: (target, property, receiver) => {
                if (property in target) {
                    ...
                    if (!fs.existsSync(newPath)) {
                        throw new Error(`The path ${newPath.yellow} doesn't exist`)
                    }
