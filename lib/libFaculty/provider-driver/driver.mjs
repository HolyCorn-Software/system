...
/**
 * Use plugins, not ProviderLoader
 * @deprecated 
 * @template ModelClass
 */
export class ProviderLoader {
     ...
     * This is deprecated.
     * 
     * Use the concept of plugins instead.
     * 
     * @param {object} param0
     ...
     * 
     */
    constructor({ providers, model, fileStructure, relModulePath, credentials_collection }, importURL = soulUtils.getCaller()) {
    ...
    async load(getParams = () => undefined, whitelist = this.providerNamesNonLoaded) {
        ...
        let caller = soulUtils.getCaller() || soulUtils.getCaller(1) // / *at .*\(([^(]+)\)$/.exec(new Error().stack.split('\n')[3])?.[1];
        if (!caller) {
     ...
     * 
     */
    async routeHTTP({ http, whitelist, credentialsURL = '/providers/credentials', static_file_url = '/static/' }, importURL = soulUtils.getCaller()) {
...
export class CredentialsError extends Exception {
    /**
