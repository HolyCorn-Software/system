...
import quickConnect from '../../database/quick.js'
...
export class FacultyPlatform extends Platform {
    ...
    /**
     * @deprecated Use faculty.connectionManager.connect instead
     * @param {string} name 
     * @returns {Promise<FacultyFacultyInterface>}
     */
    connectFaculty(name) {
        console.warn(`faculty.connectFaculty() is deprecated. Use faculty.connectionManager.connect() instead\n${new Error().stack.split('\n').slice(2).join('\n')}`)
        return this.connectionManager.connect(name)
    }

    /**
     * @deprecated
     * Just use console.warn
     * @param {string} message 
     */
    warn(message) {

        console.log(`\n\n\t\t${this.descriptor.label.underline}\n\n\t${message}\n${new Error().stack}`.black.bgYellow)
