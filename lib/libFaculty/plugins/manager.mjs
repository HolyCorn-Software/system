...
export default class PluginManager {
    ...
    async init() {
        ...
        const loadResults = await Promise.allSettled(
            preloadSuccess.filter(suc => enabledPlugins.indexOf(suc.value.descriptor.name) !== -1).map(async success => {
                ...
                if (!results.error) {
                    await this.invokeStart(results)
                }
    ...
    async loadPlugin(preloadData) {
        ...
        try {
            ...
            if (!PluginModelClass) {
                throw new Error(`Unfortunately some plugins will not load, because the model "${pluginDefinition[namespace].model?.red}" was not found. Check the spelling of the model, and check that the class is globally available`)
            }
