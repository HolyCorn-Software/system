...
const loadPromise = Symbol()

const init = Symbol()


/**
 * @template {import("./types.js").DefaultNamespaceMap} PluginNamespaceMap 
 */
export default class PluginManager {
    ...
    constructor() {
        ...
        /**
         * @type {import("./types.js").PluginLoadResultMap<PluginNamespaceMap>}
         */
        this[registry] = {
        ...
        this.loaded = new PluginList(this)

        /** @deprecated */ this.plugins;

        Reflect.defineProperty(this, 'plugins', {
            get: () => {
                console.trace(`The PluginManager field 'plugins', is deprecated. Use 'loaded'`)
                return this.loaded
            },
            configurable: true,
            enumerable: true
    ...
    /**
     * If components want to do something particular, only after the plugins have loaded,
     * then they can await this method
     */
    async waitForLoad() {
        try {
            let timeout;
            ...
            await Promise.race(
                [
                    new Promise(async resolve => {
                        //Here, we are waiting for the process of loading plugins to begin
                        while (!this[loadPromise] && isWaiting) {
                            await new Promise(x => setTimeout(x, 100))
                        }
                        resolve()
                    }),
                    //Should in case plugins have not yet started loading, we issue a warning
                    new Promise(resolve => {
                        timeout = setTimeout(() => {
                            resolve()
                            console.log(`The process that is waiting for plugins to load, is blocking plugins from loading.\nUnfortunately! \n${stack}`)
                        }, 10_000)
                    })
                ]
            )
            ...
            //Here, we are waiting for the plugins to actually load
    ...
    async [init]() {
        ...
        /** @type {PromiseRejectedResult<import("./types.js").PluginPreLoadResult>[]} */
        ...
        /** @type {PromiseFulfilledResult<import("./types.js").PluginPreLoadResult>[]} */
        ...
        const loadResults = await Promise.allSettled(
            preloadSuccess.filter(prSuc => enabledPlugins.indexOf(prSuc.value.descriptor.name) !== -1).map(async success => {
        ...
        for (let plugin of disabled) {
            const results = preloadResults.find(x => x.value.descriptor.name === plugin).value
        ...
        for (const task of errorTasks) {
            ...
            console.error(`${task.value?.descriptor.name.red}\n`, errorObject.stack || errorObject)
        }
        ...
        const stopPlugins = () => {
            for (const plugin of this.loaded.$all) {
                plugin._stop()
            }
        }
        process.addListener('SIGTERM', stopPlugins)
        process.addListener('SIGINT', stopPlugins)

        // Now, let's watch for changes to the directory structure, in order to automatically add new plugins
        const watcher = chokidar.watch(this.pluginsDir, { depth: 1 })
        let timeouts = {}
        setTimeout(() => watcher.on('addDir', (path) => {

            const nwPluginName = libPath.relative(this.pluginsDir, path).split('/')[0]

            clearTimeout(timeouts[nwPluginName]);



            timeouts[nwPluginName] = setTimeout(() => {

                const realPath = `${this.pluginsDir}/${nwPluginName}`

                for (const entryName in this[registry]) {
                    if ((this[registry][entryName].path == realPath) && (this[registry][entryName].state !== 'crashed')) {
                        return
                    }
                }

                delete timeouts[nwPluginName]
                this.preloadPlugin(nwPluginName).then(x => this.loadPlugin(x)).then(x => {
                    this[registry][x.descriptor.name] = x
                }).catch(e => {
                    console.error(`Failed to automatically load new plugin in path ${realPath.red}\n`, e)
                })
            }, 3000)
        ...
        console.log(`Public files for plugins may be accessed via https://${faculty.server_domains.secure}${faculty.standard.httpPath}$plugins/${'pluginName'.magenta}/@public/`.yellow)
    ...
    async installPlugin({ url }) {
        ...
        await new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                const uz = unzipper.Extract(
                    {
                        path: tmp
                    }
                );
                response.body.pipeTo(uz)
                uz.addListener('close', () => resolve())
                uz.addListener('error', (e) => reject(e))
            } catch (e) {
                failed(e)
            }
        })
    ...
    async loadPlugin(preloadData) {
        ...
        try {
            if (preloadData.state == 'crashed') {
                return preloadData
            }
            if (!pluginDefinition[namespace]) {
                throw new Exception(`The plugin ${preloadData.descriptor.name} references a non-existent namespace: ${namespace}`)
            }
        ...
        if (descriptor.node_modules) {
            ...
            await Promise.all(
                descriptor.node_modules.map(async (depen) => {
                    try {
                        await npmManager.install(depen, 'latest')
                    } catch (e) {
     ...
     * @param {import("./types.js").PluginLoadResult<PluginModelModel>} preloadData 
     * @returns {Promise<void>}
     */
    ...
    async setCredentials({ plugin, credentials }) {
        ...
        if (!pluginData) {
            console.trace(`pluginData is `, pluginData)
        ...
        pluginData.instance?.reloadCredentials()

        if (pluginData.state == 'crashed') {
    ...
    async uninstall({ plugin }) {
        ...
        await fs.promises.rm(`${this.pluginsDir}/${plugin}`, { recursive: true, force: true })
