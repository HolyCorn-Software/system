...
export class Router {
    ...
    */
    bestContract(url) {
        ...
        /**
         * The longest path is the most precise one
         * @param { Router['contracts']} array 
         */
        function longest(array) {
            return array.sort((x, y) => {
                function length(c) {
                    return (c.path || '').split('/').length
                }
                let nX = length(x), nY = length(y);
                return nX > nY ? 1 : nX == nY ? 0 : -1
            }).at(-1)
        }
        return longest(this.contracts.filter(c => c.point == clean_url))
            //Then the one whose url matches a certain pattern
            || longest(this.contracts.filter(c => c.regexp?.test(clean_url)))
            //Then finally, the one in the path
            || longest(this.contracts.filter(c => clean_url.startsWith(c.path)))
    }
