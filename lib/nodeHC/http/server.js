...
export class Server extends Router {
    ...
    set server(server) {
        /** Each time the 'http' attribute is being set, we setup listeners */
        ...
        server.on('request', this.serve.bind(this));
    }
    get server() {
    ...
    /**
     * Set up actions that trigger once a websocket connection is received
     */
    setupWebSockets() {
            ...
            this.route({
                ...
                callback:
                    ...
                    (request, response) => {
                        ...
                        //Once the whole request has been read from the client
                        request.on('end', async () => {
                            ...
                            try {
                                ...
                                let data = await fetch(`${remoteURL}${request.url.replace(localPath, '')}`, {
                                ...
                                let readBuffer = new Int8Array(await data.arrayBuffer())
                                let buf = Buffer.from(readBuffer)
