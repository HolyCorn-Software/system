...
class LooperStream extends BufferedStream {
    ...
    _write(chunk, encoding, callback) {
        ...
        this[input_buffer] = Buffer.concat([this[input_buffer], Buffer.from(chunk)])
        callback()
        this.emit('write-end')
    ...
    throwAway() {
        const nwBuff = Buffer.alloc(this[input_buffer].byteLength - this[input_buffer_offset], 0)
        this[input_buffer].copy(nwBuff, 0, this[input_buffer_offset], this[input_buffer].byteLength + 1)
        this[input_buffer] = nwBuff
        this[input_buffer_offset] = 0;
    ...
    waitTillLength(byteLength, timeout) {
        const timeout_error = new Error(`Timeout waiting for data to reach length ${byteLength}. Data length is ${this[input_buffer].byteLength} `)
        ...
        return new Promise((resolve, reject) => {
            ...
            if (is_okay()) {
                ...
                intervalKey = setInterval(() => {
                    ...
                    if (this.failed) {
                        return fail(new Error('Stream destroyed'))
                    }
...
class DecoderStream extends LooperStream {
            ...
            try {
                ...
                const read_once = async () => {
                    ...
                    if (data_chunks.length > 0 && results.header.type !== 'continue') {
                        throw new Error(`A frame came in, and we don't know where to put it. We expected a continuation, but ${results.header.type} of length ${results.header.payloadLength}, came, with header ${printBuffer(results.header.buffer)}`)
                    }
                    ...
                    if (!results.header.fin) {
                        await this.waitTillLength(this[input_buffer_offset] + 8)
    ...
    async [readHeader]() {
        ...
        const first_bytes = [this[input_buffer].readUInt8(this[input_buffer_offset] + 0), this[input_buffer].readUInt8(this[input_buffer_offset] + 1)]

        const header_buffer = Buffer.alloc(16, 0)
        let header_buffer_offset = 0;
        first_bytes.forEach((x, i) => header_buffer.writeUint8(x, i))
        header_buffer_offset += 2
        ...
        if (payloadLength === 126) {
            ...
            payloadLength = this[input_buffer].readUint16BE(this[input_buffer_offset])
            header_buffer.writeUint16BE(payloadLength, header_buffer_offset)
            header_buffer_offset += 2
            this[input_buffer_offset] += 2
        } else {
            ...
            if (payloadLength === 127) {
                ...
                payloadLength = Number(this[input_buffer].readBigUInt64BE(this[input_buffer_offset])).valueOf()
                this[input_buffer_offset] += 8
                header_buffer.writeBigUInt64BE(BigInt(payloadLength))
                header_buffer_offset += 8
            }
        ...
        if (isMasked) {
            ...
            mask_key.copy(header_buffer, header_buffer_offset)
            header_buffer_offset += 4;
        }
...
class EncoderStream extends BufferedStream {
    ...
    static createSocketFrame({ source, type, isLast = true }) {
        ...
        header_buffer.writeUInt8(
            (source.byteLength >= 126 ? 126 : source.byteLength),
            offset++
        )
...
function printBuffer(buffer) {
    ...
    return [...(buffer.toString('hex').matchAll(/../gi))].join(' ')
}