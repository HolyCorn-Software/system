...
class LooperStream extends BufferedStream {
    ...
    async tick() {
        //Wait till, there's at least 2 bytes for the header
        if (this[is_ticking]) {
            return;
        }
        ...
        try {
            ...
            if (!/timeout/gi.test(`${e}`)) { //Timeout errors are normal, so we ignore them
        ...
        if (this[input_buffer].byteLength > this[input_buffer_offset]) {
            this.tick()
        }
...
class DecoderStream extends LooperStream {
    ...
    constructor(options) {
        ...
        this.on('end', () => {
            console.log(`Ending an decoder stream.`)
        })
        ...
        await new Promise((resolve, reject) => {
            ...
            try {
                ...
                const read_once = async () => {
                    ...
                    const timeout_error = new Error(`Timeout waiting for the next socket frame `)
                    timeout = setTimeout(() => {
                    ...
                    this.throwAway(this[input_buffer_offset])
    ...
    async [readHeader]() {
        ...
        if (payloadLength === 126) {
            ...
            await this.waitTillLength(2 + this[input_buffer_offset], 1000)
            ...
            if (payloadLength === 127) {
                ...
                await this.waitTillLength(8 + this[input_buffer_offset], 1000)
                payloadLength = this[input_buffer].readBigUInt64BE(this[input_buffer_offset])
    ...
    async [readPayloadData]({ length, mask }) {
        ...
        await this.waitTillLength(this[input_buffer_offset], 1000)
